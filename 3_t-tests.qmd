...
```{r}
# Base R version:
t_results = t.test(data_for_t_test$body_mass_g ~ data_for_t_test$species)
t_results
class(t_results)
summary(t_results)
t_results$statistic
...
data_for_t_test %>% 
  t_test(body_mass_g ~ species) # var.equal = T yields Student's t-test
```
...
In the situations where the assumptions are violated, non-parametric tests, such as the Wilcoxon test, are recommended. Note that parametric tests (like Student's t-test) make assumptions about the parameters of the underlying data, i.e. that they are normally distributed. Non-parametric tests (like Welch's t-test) to not rely on assumptions about the data distribution, and instead operate on the ordered ranks of the dataset.
